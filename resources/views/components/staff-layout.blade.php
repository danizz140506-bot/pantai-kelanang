...
@php
    $role = auth()->user()->role;
    $links = [['label' => 'Dashboard', 'route' => 'dashboard', 'active' => 'dashboard', 'icon' => 'M4 5h6v6H4zM14 5h6v4h-6zM14 13h6v6h-6zM4 15h6v4H4z']];
    if (in_array($role, ['Waiter', 'Owner'])) {
        $links[] = ['label' => 'Tables', 'route' => 'tables.index', 'active' => 'tables.*', 'icon' => 'M4 7h16M6 7v10M18 7v10M4 17h16'];
        $links[] = ['label' => 'Orders', 'route' => 'orders.index', 'active' => 'orders.index', 'icon' => 'M8 6h11M8 12h11M8 18h11M4 6h.01M4 12h.01M4 18h.01'];
    }
    if (in_array($role, ['Kitchen Staff', 'Owner'])) {
        $links[] = ['label' => 'Kitchen Display', 'route' => 'kds.index', 'active' => 'kds.*', 'icon' => 'M5 21V10m14 11V10M3 10h18M12 3v3m-4 0V4m8 2V4'];
    }
    if (in_array($role, ['Cashier', 'Owner'])) {
        $links[] = ['label' => 'Billing', 'route' => 'billing.index', 'active' => 'billing.*', 'icon' => 'M3 7h18v10H3zM3 11h18'];
    }
    if ($role === 'Owner') {
        $links[] = ['label' => 'Reports', 'route' => 'reports.index', 'active' => 'reports.*', 'icon' => 'M4 19V5m0 14h16M8 15v-4m4 4V9m4 6v-6'];
    }
@endphp
    ...
    <body class="min-h-screen bg-espresso-950 font-sans text-cream antialiased" x-data="{ sidebar: false }">
        ...
        <div class="sticky top-0 z-30 flex items-center justify-between border-b border-espresso-700 bg-espresso-950/95 px-4 py-3 backdrop-blur lg:hidden">
            <span class="flex items-center gap-2">
                ...
                <span class="font-display text-base font-semibold text-cream">Pantai Kelanang</span>
            </span>
        ...
        <aside class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col border-r border-espresso-700 bg-espresso-900 transition-transform duration-200 lg:translate-x-0"
            ...
            <div class="flex items-center gap-3 border-b border-espresso-700 px-5 py-5">
                ...
                <div class="min-w-0">
                    <p class="text-[9px] font-semibold uppercase tracking-[0.2em] text-ember">Restaurant Mgmt</p>
                    <p class="truncate font-display text-sm font-semibold text-cream">Pantai Kelanang</p>
                </div>
            </div>
            ...
            <div class="border-t border-espresso-700 p-3">
                <div class="flex items-center gap-3 rounded-lg px-3 py-2">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-[11px] text-cream-faint">{{ $role }}</p>
                    </div>
        ...
        <div class="lg:pl-64">
            @isset($header)
                <header class="border-b border-espresso-700 bg-espresso-900/40">
